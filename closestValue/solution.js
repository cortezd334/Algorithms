...
const helper = (tree, target, min) => {
    console.log( tree, target, min)
    let current = tree.value
    let left = tree.left
    let right = tree.right

    if(tree === null) return min;
    if(Math.abs(target - min) > Math.abs(target - current)){
        min = current
    }
    if(target > current){
        return helper(right, target, min)
    } else if(target < current){
        return helper(left, target, min)
    } else {
        return min
    }
}
...
console.log(findClosestValueInBst({"tree": {
    "nodes": [
